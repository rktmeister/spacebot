...
Spacebot sends Telegram replies using Telegram's HTML parse mode, not full GitHub-flavored Markdown. The reliable subset is:
...
Spacebot also flattens unsupported markdown tables into Telegram-safe bullet lines instead of sending raw pipe-table text.

Spacebot also applies a small Telegram-specific cleanup pass before rendering so obviously collapsed inline lists such as `by:1. ...2. ...`, compact date/time text such as `March13,2026 at2:00 PM`, glued sentence starters such as `...Then the review started`, glued section labels such as `...SummaryAction items:`, and collapsed heading/list seams such as `**Heading:**- item` are rewritten into more readable Telegram-safe text. Inline code spans are preserved literally while adjacent prose still gets repaired.
