---
title: Email Setup
description: Connect Spacebot to an inbox with IMAP and optional SMTP.
---

# Email Setup

Connect Spacebot to any mailbox that supports IMAP. Inbound messages are polled over IMAP. SMTP is optional and is only needed when you want intentional outbound email delivery.

You always need:

- an IMAP host, username, and password

For outbound email sending, you also need:
...
If you want outbound email too, add the SMTP fields:
...
Credentials support `env:VAR_NAME` references. `allow_outbound` defaults to `true`, so set it to `false` if you want IMAP-only ingest with no SMTP sending capability.
...
When the Email adapter is configured with `allow_outbound = true`, you can intentionally send email from a non-email channel (for example, Telegram or Discord) using cross-channel messaging.

- explicit format: `email:alice@example.com`
- bare address also works: `alice@example.com`

This keeps email channels inbound-only while still allowing deliberate outbound send workflows when enabled.
...
Search results keep the mailbox's underlying UTC/reference time available, but the human-facing `date` field is rendered in the agent's resolved user timezone (`agents.user_timezone` / `defaults.user_timezone`, falling back to the cron timezone or system local time).

## Calendar invites (`.ics`)

Spacebot now extracts meeting details from inbound calendar invite attachments without broadly enabling arbitrary attachment execution.

- Only calendar invite attachments are parsed (`.ics`, `text/calendar`).
- Invite metadata is added to the email body context so the agent can read back exact meeting times.
- Times are normalized into both the invite's local timezone and UTC when the timezone is present.
- The raw invite file is kept as an attachment reference for later recall when attachment saving is enabled.

This is intentionally narrow:

- Spacebot does not execute email attachments.
- Non-calendar email attachments are still treated as filenames/metadata only.
- Calendar invites are not turned into reminders automatically. The agent can create a cron reminder after it sees the parsed meeting time, but that remains an explicit action.
