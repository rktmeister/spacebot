...
| `allow_channel_replies` | bool | true | Allow normal Spacebot replies/broadcasts over the email adapter |
...
When `allow_channel_replies = false`, Spacebot can still poll IMAP and use the configured SMTP credentials for calendar invites, but it will not send normal conversational replies over the email adapter.

`[messaging.email]` can also be configured as SMTP-only for calendar invite delivery. When `imap_host` is left empty, Spacebot does not start the email adapter or poll any inbox, but calendar invite delivery can still use the configured SMTP credentials.
...
| `name` | string | **required** | Instance selector used by bindings (`adapter = "name"`) |
| `enabled` | bool | true | Enable this named instance |
| `allow_channel_replies` | bool | true | Allow normal Spacebot replies/broadcasts over this email adapter |
