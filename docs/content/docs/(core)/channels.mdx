...
Channels are aware of each other. On every turn, the channel's system prompt is injected with a list of all active user-facing channels (excluding the current one, cron jobs, webhooks, and internal link channels). This gives the LLM context about where it can send messages.
