...
- **Email** — IMAP polling for inbound, SMTP for outbound. Each email thread maps to a conversation.
- **Matrix** — decentralized chat protocol support. Bridges to self-hosted Matrix/Element deployments.

### Onboarding CLI

Interactive setup wizard for self-hosted instances.

- `spacebot setup` — walks through API key config, provider selection, first agent creation, channel connection
- `spacebot setup --quick --provider openrouter --key sk-...` — non-interactive fast path
