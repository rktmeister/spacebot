...
| `delivery_target` | Where to send results, format `adapter:target`. Tool-created jobs inside a live conversation pin this to the current conversation automatically. |
...
| Format | Meaning |
|--------|---------|
| `discord:<channel_id>` | Send to a Discord channel |
| `telegram:<chat_id>` | Send to a Telegram chat |
| `webhook:<endpoint>` | Send via webhook adapter |
...
In this conversational path, the tool ignores mismatched `delivery_target` overrides and pins delivery to the current conversation target. The tool persists to the database and registers with the running scheduler immediately — no restart needed.
