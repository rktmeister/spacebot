...
Human-facing timestamps from `channel_recall` are rendered in the resolved user timezone, while explicit UTC companion fields are preserved for exact chronology.
...
The primary `date` field is rendered in the resolved user timezone so branches reason about "today" and "this morning" using the same wall clock the user sees. UTC is still preserved separately for exact inspection.
