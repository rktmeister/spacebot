...
organizer_name = "Spacebot"
organizer_email = "secret:CALENDAR_ORGANIZER_EMAIL"
...
- attendee-bearing events stored via CalDAV should not persist an iCalendar `METHOD`; scheduling messages are generated by the CalDAV server from organizer resources
- `smtp_invites_enabled = true` tells Spacebot to send invite and cancel emails itself using the configured SMTP credentials instead of relying solely on CalDAV server-side scheduling; update flows should send `CANCEL` to removed attendees and refreshed `REQUEST` messages to the remaining attendee set
