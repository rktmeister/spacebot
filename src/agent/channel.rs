...
impl Channel {
    ...
    async fn handle_message_batch(&mut self, messages: Vec<InboundMessage>) -> Result<()> {
        ...
        self.handle_agent_result(
            ...
            &turn_result.turn_output_capture,
            false,
        )
        .await;
        if turn_result
...
mod tests {
    use super::{
        CronAsyncState, ObserveModeFallbackState, compose_channel_adapter_prompt,
        compute_listen_mode_invocation, cron_can_finalize, decision_user_id,
        extract_decision_summary_from_reply, is_cron_channel_id, is_dm_conversation_id,
