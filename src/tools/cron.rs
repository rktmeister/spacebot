...
impl CronTool {
    async fn create(&self, args: CronArgs) -> Result<CronOutput, CronError> {
        ...
        let conversation_default_target = self
            .default_delivery_target
            .as_deref()
            .map(str::trim)
            .filter(|value| !value.is_empty());
        let delivery_target = self.resolve_delivery_target(&args)?;
        let explicit_target_was_applied =
            conversation_default_target.is_none() && explicit_delivery_target.is_some();
        let id = args
            .id
        ...
        let delivery_target = if explicit_target_was_applied {
